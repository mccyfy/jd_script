...
function start() {
	...
	//开团
	let body = {"activityIdEncrypted":"hYzN_L8AA_o=\n","channel":"NINE_BOX"}
	//console.log(taskOpenKaiUrl("distributeBeanActivityInfo", body));
	return new Promise(resolve => {
	$.get(taskOpenKaiUrl("vvipclub_distributeBean_startAssist", body), async (err, resp, data) => {
	  try {
		if (err) {
...
function openKai() {
  let body = {"paramData":{"channel":"NINE_BOX"}}
  //console.log(taskOpenKaiUrl("distributeBeanActivityInfo", body));
  return new Promise(resolve => {
    $.get(taskOpenKaiUrl("distributeBeanActivityInfo", body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
          ...
          if (safeGet(data)) {
            ...
            if (data['success']) {
              $.hasOpen = true
			  myList.push(...[data.data['assistStartRecordId']+"&"+data.data['encPin']]);
            }
          }
        }
      } catch (e) {
...
function helpMyTuan(myString) {
	//console.log(myString)
  let list=myString.split('&');
  let body = {"activityIdEncrypted":"hYzN_L8AA_o=\n","assistStartRecordId":list[0],"assistedPinEncrypted":list[1]+"\n","channel":"NINE_BOX"}
  return new Promise(resolve => {
    $.get(taskOpenKaiUrl("vvipclub_distributeBean_assist", body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
